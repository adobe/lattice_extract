...
struct KernighanLinSettings {
    unsigned long numberOfInnerIterations { std::numeric_limits<unsigned long>::max() };
    unsigned long numberOfOuterIterations { 100 };
    double epsilon { 1e-6 };
    bool verbose { false };
    ...
    const KernighanLinSettings settings = KernighanLinSettings()) -> VERTEXLABELS
{
    struct Buffers {
        ...
        unsigned long max_not_used_label;
        std::vector<unsigned long> referenced_by;
        VERTEXLABELS vertex_labels;
    } buffer(original_graph);

    auto update_bipartition = [&buffer, &original_graph, &lifted_graph, &edge_costs, &settings] (std::vector<unsigned long>& A, std::vector<unsigned long>& B)
    {
        struct Move
        {
            int v { -1 };
        ...
        auto compute_differences = [&](const std::vector<unsigned long>& A, unsigned long label_A, unsigned long label_B)
        {
        ...
        std::pair<double, unsigned long> max_move { std::numeric_limits<double>::lowest(), 0 };

        for (unsigned long k = 0; k < settings.numberOfInnerIterations; ++k)
        {
            Move m;
            ...
            {
                unsigned long size = border.size();
        ...
        {
            // revert some changes
            for (unsigned long i = max_move.second; i < moves.size(); ++i)
            {
            ...
            A.erase(std::partition(A.begin(), A.end(), [&](unsigned long a) { return !buffer.is_moved[a]; }), A.end());
            B.erase(std::partition(B.begin(), B.end(), [&](unsigned long b) { return !buffer.is_moved[b]; }), B.end());

            for (unsigned long i = 0; i < max_move.second; ++i)
    ...
    auto compute_obj_value = [&lifted_graph, &edge_costs] (std::vector<unsigned long> const& vertex_labels)
    {
        double obj_value = .0;

        for (unsigned long i = 0; i < lifted_graph.numberOfEdges(); ++i)
        {
    ...
    auto form_partitions = [] (std::vector<unsigned long> const& vertex_labels)
    {
        auto const number_of_components = *std::max_element(vertex_labels.begin(), vertex_labels.end()) + 1;

        std::vector<std::vector<unsigned long>> partitions(number_of_components);

        for (unsigned long i = 0; i < vertex_labels.size(); ++i)
            partitions[vertex_labels[i]].push_back(i);
        
        return partitions;
    };

    auto project_onto_feasable_set = [&original_graph] (std::vector<unsigned long> const& vertex_labels)
    {
        std::stack<unsigned long> S;
        std::vector<char> visited(original_graph.numberOfVertices());

        std::vector<unsigned long> feasable_vertex_labels(vertex_labels.size());

        // do connected component labeling on the original graph
        for (unsigned long i = 0, new_label = 0; i < original_graph.numberOfVertices(); ++i)
            if (!visited[i])
            {
                S.push(i);
                visited[i] = 1;
    ...
    auto mark_partitions_that_changed_shape = [&original_graph] (std::vector<unsigned long> const& previous_vertex_labels, std::vector<unsigned long> const& current_vertex_labels)
    {
        ...
        for (unsigned long i = 0; i < original_graph.numberOfVertices(); ++i)
            if (!visited[i])
            {
                S.push(i);
                visited[i] = 1;
    ...
    for (unsigned long k = 0; k < settings.numberOfOuterIterations; ++k)
    {
        ...
        std::vector<std::set<unsigned long>> edges(partitions.size());
        for (unsigned long e = 0; e < original_graph.numberOfEdges(); ++e)
        {
        ...
        for (unsigned long i = 0; i < partitions.size(); ++i)
        ...
        if (settings.introduce_new_sets)
            for (unsigned long i = 0; i < partitions.size(); ++i)
            {
                ...
                while (1)
                {
                    std::vector<unsigned long> new_set;
...
void kernighanLin(const ORIGINAL_GRAPH& original_graph, const LIFTED_GRAPH& lifted_graph, const ECA& edge_costs, const ELA& input_edge_labels, ELA& output_edge_labels, const KernighanLinSettings settings = KernighanLinSettings())
{
    struct Visitor
    {
        bool operator()(std::vector<unsigned long> const& vertex_labels) const
        {
            return true;
        }
    } visitor;
...
{
    std::vector<unsigned long> vertex_labels(lifted_graph.numberOfVertices());
    std::vector<char> visited(lifted_graph.numberOfVertices());

    std::stack<unsigned long> S;
    for (unsigned long i = 0, label = 0; i < lifted_graph.numberOfVertices(); ++i)
        if (!visited[i])
        {
            S.push(i);
            visited[i] = 1;
    ...
    for (unsigned long e = 0; e < lifted_graph.numberOfEdges(); ++e)
    {
