...
std::vector<unsigned long> greedyAdditiveEdgeContraction(
    ORIGGRAPH const& original_graph,
    LIFTGRAPH const& lifted_graph,
    ECA const& edge_costs,
    size_t number_of_clusters_lower_bound = 1,
    size_t number_of_clusters_upper_bound = std::numeric_limits<unsigned long>::max()
)
{
    class DynamicGraph
    {
    public:
        DynamicGraph(unsigned long n) :
            vertices_(n)
        {}

        bool edgeExists(unsigned long a, unsigned long b) const
        {
            return !vertices_[a].empty() && vertices_[a].find(b) != vertices_[a].end();
        }

        std::map<unsigned long, typename ECA::value_type> const& getAdjacentVertices(unsigned long v) const
        {
            return vertices_[v];
        }

        typename ECA::value_type getEdgeWeight(unsigned long a, unsigned long b) const
        {
            return vertices_[a].at(b);
        }

        void removeVertex(unsigned long v)
        {
        ...
        void setEdgeWeight(unsigned long a, unsigned long b, typename ECA::value_type w)
        {
            vertices_[a][b] = w;
            vertices_[b][a] = w;
        }
    ...
    private:
        std::vector<std::map<unsigned long, typename ECA::value_type>> vertices_;
    };

    struct Edge
    {
        Edge(unsigned long _a, unsigned long _b, typename ECA::value_type _w)
        {
            if (_a > _b)
                std::swap(_a, _b);
    ...
    std::vector<std::map<unsigned long, unsigned long>> edge_editions(original_graph.numberOfVertices());
    ...
    for (size_t i = 0; i < lifted_graph.numberOfEdges(); ++i)
    {
        ...
        {
            printf("original edge doesn't exist!\n");
        }
    }

    andres::Partition<unsigned long> partition(original_graph.numberOfVertices());
    ...
    std::map<unsigned long, unsigned long> indices;
    for (unsigned long v = 0; v < original_graph.numberOfVertices(); ++v)
    ...
    std::vector<unsigned long> vertex_labels(original_graph.numberOfVertices());
    for (unsigned long v = 0; v < original_graph.numberOfVertices(); ++v)
